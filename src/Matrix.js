...
export default class Matrix {
  ...
  getColumn(column) {
    let result = [];
    for (let row = 0; row < this._rows; ++row) {
      result[row] = this._matrix[row * this._columns + column];
    }
    return result;
  }

  normalizeAndScale(scaler = 1) {
    let min = Infinity, max = -Infinity;
    this._matrix.forEach(x => {
      if (x < min) min = x;
      if (x > max) max = x;
    });

    this._matrix.forEach((x, i, arr) => {
      arr[i] = scaler * (x - min) / (max - min)
    });
  }
